...
> 架构白板常给 **开放式业务场景**（设计 IM / 外卖 / 企业知识平台），45 分钟练习 **澄清 → 估算 → MVP → 扩展 → 非功能 → 演进**。本模板整合 [03 系统设计](../03-system-design/index.md) 单场景能力，形成 **完整叙事**；不是新知识点，而是 **白板交付结构**。

## 3 分钟版（精讲深度）

1. **是什么**：可复用的白板时间盒与话术，展示 Staff/架构师级结构化思维。
2. **为什么**：架构实践者差在「散点懂很多，45min 讲不完整」；本模板是 **架构师岗必练**。
...
- 平均 QPS = 活跃用户 × 观察周期内人均请求 / 周期秒数；峰值应优先来自历史曲线、
  业务事件和地域分布。没有数据时再声明一个假设系数并做 sensitivity，不能把
  `3～10` 当行业常量
- 存储 = 日增 × 保留天数 × 副本系数
- 带宽 = QPS × 平均响应体
...
|------|------|
| 读 | 缓存、CDN、读副本 |
| 写 | 先定位锁/索引/事务/热点；再考虑批处理、分片或异步派生。需要同步确认的核心写不能为提吞吐随意异步化 |
| 热点 | 本地缓存、请求合并 |
| 单点 | 冗余只是第一步；还要有 quorum/leader fencing、故障转移、数据恢复与演练 |
...
| 阶段 | 目标 | 架构变化 |
|------|------|----------|
| MVP | 最小闭环 | 从业务不变量出发选择最少组件；不默认必须有 Redis |
| 10x | 已验证瓶颈 | 针对读、写、热点或 fan-out 的证据做缓存、批处理、队列或分片 |
| 100x / 多区域 | 地域延迟或隔离需求明确 | 评估分片、单元化、主备或多活；多活不是规模增长后的自动终点 |
...
- 「最大风险要与题目不变量对应；若订单写必须同步确认，先做幂等、容量与降载，
  异步化仅用于可延迟的派生处理，对账是检测/修复防线而不是一致性的替代品」
- 「回滚策略是特性开关 + 网关切流」

...
<a id="sweep-vs-withdraw"></a>

**归集 vs 用户提现：热钱包争用**

归集是独立资金工作流，不是提现状态机里的一个 if。两者常争用同一热钱包的原生币、nonce/UTXO 与签名队列。

| 机制 | 做什么 |
|------|--------|
| 预算与水位 | 为用户提现保留最低热钱包余额 / 日额度；归集不得抽干提现池 |
| 队列优先级 | 用户提现 > 运营归集；拥堵时延后或降频 sweep |
| 分地址/分账户 | 大额归集走独立 hot 或中转地址，减少与提现同 nonce 域冲突 |
| 熔断 | 提现 SLA 告警或 UNKNOWN 积压时，自动暂停非必要归集 |
| 对账 | 归集、top-up、提现在途分列，禁止只看链上热钱包总额 |
...
2. **归集是否越频繁越好？** → 否；要权衡热钱包暴露、手续费、碎片和提现流动性。见 [争用](#sweep-vs-withdraw)。
...
7. **归集把热钱包打空导致提现失败？** → 用提现保留水位 + 优先级队列 + 熔断；归集与提现分预算，而不是共享一个“有钱就扫”。
...
- 高峰归集与用户提现抢同一 nonce 域且无优先级，造成提现 SLA 崩溃。

...
5 篇 | 协议与共识深挖 | [返回专题索引](../../topic-catalog.md)
...
| ID | 标题 | 频率 |
|----|------|------|
| [S-PROTO-05](./S-PROTO-05-classic-vs-onchain-consensus.md) | 经典共识 vs 链上共识：Paxos/Raft/PBFT 与 PoW/PoS/DPoS/BFT | ⭐⭐⭐⭐⭐ |
...
1. **S-PROTO-05**：先对齐经典共识与链上共识的成员模型、故障假设与「确认」语义。
2. **S-PROTO-01**：再分清 head、justified、finalized 与 weak-subjectivity checkpoint。
3. **S-PROTO-02**：再比较链式 PoS fork choice 与按高度多轮 BFT。
4. **S-PROTO-03**：理解 Rollup 依赖的 DA 不只是“把数据哈希上链”。
5. **S-PROTO-04**：把协议规则、节点二进制、链上状态迁移和本地数据库迁移拆开。
...
| [S-DIST-02 分布式锁](../middleware/redis/S-DIST-02-distributed-lock.md) | 对照 Raft 提交与链上最终性不是同一水位 |

## 能力自测

- 能否用一句话说清 Raft 提交、PoW N 确认、Ethereum finalized、CometBFT commit 的差别？

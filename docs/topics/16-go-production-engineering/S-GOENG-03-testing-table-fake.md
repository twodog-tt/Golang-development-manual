...
| 记忆槽 | 内容 |
|--------|------|
| 三个不变量 | 测试必须确定且可重复；断言业务契约而非实现偶然；Test Double 类型由要验证的风险决定 |
| 手画图 | `use case → small port → fake/stub/mock | real integration`，外层补少量 E2E |
| 项目落点 | 用实际账本、状态机或链适配器说明纯业务测试、数据库集成测试和外部 sandbox 如何分层；只引用本人实际参与部分和可解释指标 |
| 一个取舍 | fake 能验证有状态协议且维护成本低；mock 精确验证交互但更易与实现细节耦合 |

**错误表达**

- ❌ “用了表驱动就代表覆盖充分；给所有子测试加 `t.Parallel` 只会更快。”
- ✅ “表驱动只是组织方式；并行测试必须隔离 fixture、环境变量和外部资源。”

**自测追问**：何时应使用 mock 验证次数/顺序？为什么 sleep 和真实公网会让测试 flaky？

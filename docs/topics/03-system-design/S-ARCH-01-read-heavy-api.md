...
> 10 万 QPS 读多写少 API 的核心是 **CDN/边缘缓存 + 有界多级缓存 + 水平扩展 + 可控回源**。命中率、P99 和单 Pod QPS 都应是压测结果，不是通用常数；设计重点是即使缓存部分失效，也不会把 DB 直接打穿。

## 3 分钟版（精讲深度）

1. **是什么**：面向 C 端或 B 端的高频查询接口（商品详情、配置、Feed），读写比通常 > 100:1，峰值 QPS 10 万+。
2. **为什么**：数据库能力取决于查询、数据集、硬件和连接模型，不能背固定 QPS；当入口规模远高于可验证的回源能力时，必须分层缓存、限流和隔离。
...
- **Cache-Aside**：读 miss 回源 DB 并回填；写通常先提交 DB，再失效缓存，并用 TTL/CDC/重试兜底。所谓“延迟双删”仍有竞态窗口，不是强一致协议。
- **热点 Key**：本地缓存、只读副本、请求合并与复制热点值；给同一逻辑 Key 随意加前缀并不会自动消除热点，还会引入多副本一致性问题。
...
1. **如何保证缓存与 DB 一致？** → Cache-Aside + TTL/CDC/重试只能做到可控的最终一致；要求线性一致的读应绕过缓存、读权威存储，或把读写纳入同一个一致性系统。

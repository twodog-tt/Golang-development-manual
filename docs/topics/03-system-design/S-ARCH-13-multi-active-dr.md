...
3. **怎么做**：同城可用同步/半同步复制把目标 RPO 压低，异地常用异步复制；但 RPO=0 必须由端到端写入确认、日志/MQ 和故障切换协议共同保证，不能只凭“同步复制”下结论。配套单元化路由、GSLB/客户端多端点与定期演练。
...
| 同城双活 | 可设计为接近 0 | 秒~分钟，取决于检测和切换 | 高 | 金融同城 |
| 异地多活 | 秒~分钟 | 秒 | 很高 | 超大厂 |
...
| 工具 | 用途 |
|------|------|
| `SHOW REPLICA STATUS`（MySQL 8+） | 复制 lag；旧版本命令为 `SHOW SLAVE STATUS` |
| 混沌工程（机房隔离） | 验证 RTO |
| 对账任务 | 双活数据漂移 |
| 多活路由配置 | 流量比例 |
...
2. **DNS 切流慢？** → 降 TTL 只能缩短部分缓存窗口，递归 DNS/客户端可能不严格遵守；关键客户端还应支持多 endpoint、健康探测和受控重试。
3. **Redis 多活？** → CRDT/异步复制，接受短暂不一致；或分片各城独立。
4. **Go 服务如何无状态多活？** → 会话 Redis；配置中心多活；对象存储跨区复制。
5. **如何量化 RPO？** → 用已确认但尚未复制/持久化的日志位置或事务数衡量；“lag 秒数 × 平均 TPS”只能做粗估，峰值和事务大小会让它失真。

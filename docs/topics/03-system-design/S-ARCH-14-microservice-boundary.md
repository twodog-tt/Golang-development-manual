...
3. **怎么做**：识别核心域（订单、支付、用户）；每个服务拥有自己的数据写权限，物理上可阶段性共享数据库集群/schema；同步还是异步由返回时必须满足的业务语义决定，不按数量教条选择。
...
| 数据所有权 | 其他服务不能绕过 owner 直接写表；跨域读用 API、投影或受控迁移方案 |
| 变更耦合 | 总是一起改的不拆 |
| 团队边界 | 服务边界应匹配稳定 ownership；一个团队可拥有多个相关服务 |
| 性能边界 | 高频本地调用不拆 RPC |

**何时不该拆**

- 小团队且没有自动化交付、可观测和 on-call 能力。
- 强一致跨表事务为主，无 Saga 能力。
- 没有独立扩缩、独立发布或隔离故障的实际需求，模块化单体已足够。
- 「先拆再说」无域建模，变成分布式单体。
...
| 共享物理 DB、逻辑 ownership | 迁移期或小团队可接受 | 多服务任意跨表读写、共同修改 schema |

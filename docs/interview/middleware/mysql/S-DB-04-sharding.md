...
3. **怎么做**：优先索引、归档、读扩展和垂直边界；必须分片时选高覆盖率且分布可控的 key。扩容常用虚拟 shard/目录映射或翻倍迁移；一致性哈希并不是关系型分片的万能方案。
...
**跨库难题**：传统分片中跨片 JOIN/事务代价高，分布式 SQL/中间件虽可支持，也需评估协调与扫描成本；常见替代是应用聚合、读模型冗余。全局分页需各片 TopK 归并或独立索引；扩容采用回填/CDC、校验、切读和停止旧写。

**中间件**：ShardingSphere、Vitess、自研路由层。GORM `dbresolver` 主要做多库/读写分离，不会自动按 `user_id` 完成业务分片；分片键路由仍需插件或显式实现。

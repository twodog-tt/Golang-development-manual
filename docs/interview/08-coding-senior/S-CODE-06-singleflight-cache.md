...
| 记忆槽 | 内容 |
|--------|------|
| 三个不变量 | singleflight 只合并同进程同 key 的重叠调用；它不是缓存/分布式锁；等待者取消不应随意取消共享事实 |
| 手画图 | `cache miss → group[key] leader → loader → done → waiters`，每个 waiter 旁接自己的 ctx |
| 项目落点 | 用实际 token 元数据、链 RPC 或配置加载说明击穿窗口、TTL jitter 和多实例边界；只引用本人实际参与部分和可解释指标 |
| 一个取舍 | 共享 loader 独立于首个请求更稳定，但无人等待时仍可能继续；计数取消更省资源却更复杂 |

**错误表达**

- ❌ “singleflight 能保证全局只查一次；`shared=true` 表示结果来自缓存。”
- ✅ “共享范围仅当前 Group/进程的重叠调用；shared 只说明调用被合并。”

**自测追问**：为什么 leader 进入后要二次查缓存？所有 waiter 都离开后 loader 是否继续应由什么语义决定？

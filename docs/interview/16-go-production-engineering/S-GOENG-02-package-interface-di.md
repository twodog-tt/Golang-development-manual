...
| 记忆槽 | 内容 |
|--------|------|
| 三个不变量 | 依赖方向必须单向；接口由使用方按需要定义；创建者负责依赖生命周期和关闭顺序 |
| 手画图 | `composition root → adapters → application → consumer-owned port`，实现依赖箭头指向接口 |
| 项目落点 | 用实际 Launchpad 类 DEX 或钱包服务说明账本、链适配器、HTTP 层如何分包与组装；只引用本人实际参与部分和可解释指标 |
| 一个取舍 | 显式构造注入易读可测但组装代码多；DI 框架省样板却可能隐藏依赖和生命周期 |

**错误表达**

- ❌ “为了可测试，每个 struct 都要先定义接口；构造函数应该返回接口。”
- ✅ “只在消费者需要替换/隔离行为时定义最小接口；实现方通常返回具体类型。”

**自测追问**：接口为什么通常放在消费者包？构造函数失败后，已经创建的部分资源由谁清理？
...
资深讲解时应先给出简单显式方案，再说明规模达到什么程度才引入生成工具。

...
3. **怎么做**：按团队时区和风险设 review 响应目标；用 **nit / optional / blocking** 分级；新人 PR 小步合并；定期 Go 专题分享。所谓“4 小时首轮”只是可能的团队目标，不是行业标准。
...
| 原则 | 说明 |
|------|------|
| 速度 | 优先小而聚焦的 PR；生成代码、重命名与复杂重构不能用固定行数一刀切 |
...
- [ ] 日志：结构化、无敏感信息、级别合理
- [ ] 测试：table-driven、边界；并发敏感包跑 `-race`，大型仓库可分层/夜间执行
- [ ] API：向后兼容、版本、文档

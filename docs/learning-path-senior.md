...
| 周 | 模块 | 阅读 | 练习 | 自测 |
|----|------|------|------|------|
| W1 | [并发与运行时](./interview/01-runtime-concurrency/index.md) | 20 题 | `basis/` + `examples/senior/` | 画 GMP；讲 2 个泄漏案例 |
| W2 | [内存与 GC](./interview/02-memory-gc/index.md) | 15 题 | `go test -race`、`-gcflags=-m` | 读一份 heap profile |
...
| 5 | [分布式与中间件](./interview/04-distributed-middleware/index.md) |
| 6 | [数据库与存储](./interview/05-database-storage/index.md) + `gorm/` |
| 7 | [网络与服务治理](./interview/06-network-governance/index.md) + `gin-example/` |
| 8 | [AI 工程](./interview/10-ai-engineering/index.md) + [工程与领导力](./interview/07-engineering-leadership/index.md) + [云原生](./interview/09-cloud-native/index.md) + [手写题](./interview/08-coding-senior/index.md) |
...
```
基础：01 并发 → 02 内存 → 08 手写题
进阶：06 网络 → middleware（MySQL→Redis→MQ→ES）→ 03 系统设计 → 09 云原生
高阶：11 解决方案架构 → 15 微服务（交易所）
专题：12 Web3 Go → 13 Solidity → 14 DEX/CEX
综合：07 工程领导力 · 10 AI 工程（JD 相关时提前）
```

**岗位速查**

- **大厂 Go 后端**：基础 + 进阶 + 03 系统设计
- **架构师岗**：高阶全模块 + 07 领导力 + 45min 白板（S-SOL-08）
- **Web3 架构师**：专题 12→13 + 03 幂等/限流 + 11 边界拆分
- **交易所工程师**：专题 14 + 15 微服务 + 12 索引/充提 + 13 DeFi 合约
...
| 周 | 模块 | 自测 |
|----|------|------|
| W1 | [12 Web3 Go](./interview/12-blockchain-web3/index.md) S-BC-01～04 | 画 EVM 账户；跑 `ethrpc` 测试 |
| W2 | 12 Web3 S-BC-05～09 | 跑 `erc20bind`；讲索引 reorg |
| W3 | [13 Solidity](./interview/13-solidity-contracts/index.md) S-SOLID-01～04 | Review 重入/Proxy 题 |
...
| 周 | 模块 | 自测 |
|----|------|------|
| W1 | [14 DEX/CEX](./interview/14-dex-cex-engineering/index.md) S-EXCH-01～03 | 画撮合→账务链路；讲复式记账 |

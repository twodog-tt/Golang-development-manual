...
type aptosFailureRoundTripper func(*http.Request) (*http.Response, error)

func (transport aptosFailureRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	return transport(request)
}

func TestAptosTransportFailuresDoNotBecomeTransactionStates(t *testing.T) {
	tests := []struct {
		name      string
		transport aptosFailureRoundTripper
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			adapter, err := NewRESTAdapter("http://127.0.0.1:1/v1", &http.Client{
...
func TestAptosWrongChainIdentityFailsClosed(t *testing.T) {
	readiness := NodeReadiness{Ready: true, ChainID: 4}
	if err := readiness.ValidateChainID("1"); err == nil {
		t.Fatal("wrong chain ID was accepted")
	}
}

func TestLocalnetAptosTransportFaultDoesNotBecomeState(t *testing.T) {
	endpoint, _, client := aptosLocalnetConfiguration(t)
